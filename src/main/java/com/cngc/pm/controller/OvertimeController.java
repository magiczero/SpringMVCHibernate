...
//	            .taskAssignee(username)//任务办理人  
	...
	public String tableAjax(@RequestParam String aoData,HttpSession httpSession) throws ParseException {
	    ...
	    for(HistoricProcessInstance hpi : hpis) {
    		...
    		String company = "21";
    		String contextPath = userUtil.getContextPath(httpSession);
	    	//获取form值
	    	if(task ==null) {
	    		...
	    		for(HistoricDetail historicDetail : formProperties) {
	    			 ...
	    			 } else if(field.getPropertyId().equals("serviceCompany")) {
	    				 company = field.getPropertyValue();
	    			 } 
	    		}
	    	} else {
	    		map.put("status_", "未审批");
	    		if(isLeader)
	    			map.put("op", "<form class=\"dynamic-form\" method=\"post\" action=\""+contextPath+"/workflow/task/complete/"+task.getId()+"\"><div class=\"title\"><input name=\"redirectAddress\" value=\"/overtime\" type=\"hidden\"></div><div class=\"text\"><button class=\"btn btn-primary\" aria-hidden=\"true\">审批</button></div></form>");
	    		else
	    	    ...
	    	    company = (String)taskService.getVariable(task1.getId(), "serviceCompany");
	    	}
				...
				map.put("company", (groupService.getById(Long.valueOf(company))).getGroupName());
...
//		ProcessInstance pi = runtimeService.createProcessInstanceQuery().processInstanceId(pid).singleResult();
        ...
        String companyid = "21";
		companyid = (String)taskService.getVariable(task.getId(), "serviceCompany");
		...
		taskMap.put("company",  (groupService.getById(Long.valueOf(companyid)).getGroupName()));
