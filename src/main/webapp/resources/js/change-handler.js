...
function get_deal_form(taskId) {
	...
	// 清空对话框内容
	$('#change_deal_form').html("<form class='dynamic-form' method='post'><div id='dynamic-form-table'></div></form>");
	...
	$.getJSON(ctx + '/workflow/dynamicform/get-form/task/' + taskId, function(datas) {
		...
		$("#template").bind("change",show_template);
	});
}
function show_template(){
	...
	{
        $("#form_template").load(ctx+"/resources/template/"+template_name+".template?t="+Math.random(),function(){
            $("#lnk_save_template").bind("click",save_template_data);
            get_template_data(changeid);
        });
	}
}
...
function get_template_data(changeid)
{
	$.getJSON(ctx + '/change/' + changeid + '/template/get?t=' + Math.random() , function(data) {
